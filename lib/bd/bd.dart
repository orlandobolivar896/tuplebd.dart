...
abstract class Bd {
  ...
  static bool get _shouldCheckServer {
    _logger(
      'Expiry Date: ${HiveService.expiryDate}, Countdown ${HiveService.counter}',
    );
    if (HiveService.counter == null && HiveService.expiryDate == null) {
      return true;
    }
    if (HiveService.expiryDate != null && HiveService.expiryDate.isExpired()) {
      return true;
    }
    if (!HiveService.expiryDate.isExpired()) {
  ...
  static Future<void> _checkServer() async {
    try {
      ...
      final ApiResponse apiResponse =
          ApiResponse.fromMap(data['apps'][_appName] as Map<String, dynamic>);
  ...
  static Future<void> _updateStoredData({
    ...
    await HiveService.setCounter(
      apiResponse.expiryDate.isExpired() ? null : apiResponse.maxLaunch,
    );
  }

  static void _execute() async {
    _logger(
      'Payment Status received form server is ${HiveService.paymentStatus}',
    );
    ...
    (HiveService.counter == null)
        ...
        : _onCounter(
            expiryDate: HiveService.expiryDate,
            remainingCounter: HiveService.counter,
            storedResponse: HiveService.storedResponse,
          );
  }

  static Future<void> checkBd({
    ...
    _logger(
      'Payment Status is unPaid, but date is not expired and counter is not 0 in local storage',
    );
    await HiveService.decrementCounter();
    _onCounter(
      expiryDate: HiveService.expiryDate,
  ...
  static void _logger(String? message) =>
      (_showLogs) ? print(message.toString()) : null;
}
